import { addToCart, decreaseFromCart, removeFromCart } from '@/Redux/AddToCartSlice/AddToCartSlice';
import { statePassData } from '@/Redux/DatapassSlice/DataPassSlice';
import { modalOpenClose } from '@/Redux/ModalSlice/ModalSlice';
import { addToDb, decreaseFromCartDb, removeFromDb } from '@/Utilities/addToCartDB';
...
import { userLogin } from '../../Redux/AuthenticationSlice/AuthenticationSlice';
...
const MainProductCard = ({ product }: { product: any }) => {
    const { authentication, allProducts, cart } = useAppSelector(state => state)
    const { loginUserInfo, isAuthenticationLoading } = authentication
    ...
    const addToFavourite = (id: any, uid: any,) => {
        ...
        if (token && uid) {
            setLoading(true)
            fetch(`https://aide-api.vercel.app/favorite/products/${id}?uid=${uid}`,
                {
                    method: "PUT",
            ...
            <div className="product_card" >
                ...
                <div>
                    <div className="card_details_content">
                        <p className="product_details_text"><span className='font-medium text-primary-text'>Product of : </span>
                            {
                        ...
                        <p className="product_details_text"><span className='font-medium text-primary-text'>Brand : </span>
                            {
                    ...
                    <div className="view_details_hide">
                        <div className="flex justify-between items-center border-t-1 border-[#CECDCD]">
                            <Link href={`/productDetails/${product?._id} `} className="view_details hover:text-aide-primary">
                                <span className="w-4">
                                    <EayIcon />
                                </span>
                                <span>View Details</span>
                            </Link>
                            <Link href={'/'} className="view_details hover:text-aide-primary">

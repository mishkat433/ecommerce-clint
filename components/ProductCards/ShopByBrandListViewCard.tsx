...
const ShopByBrandListViewCard = ({ brand, shadow }: any) => {
    ...
    return (
        <div className={`group relative bg-white ${shadow && "shadow-sm"} p-2 rounded-lg w-[280px] border-[1px] border-border-color cursor-pointer`}>
            {
                <div onClick={() => {
                ...
                >
                    ...
                    <div className="">
                        {
                            name ? <h2 className="text-[13px] text-start font-medium leading-[18px] text-primary-text">{name.length > 45 ? name.slice(0, 45) + "..." : name}</h2> :
                                <h4 className="text-[13px] font-medium leading-[18px] text-red-500"> Not Found</h4>
                        }
                        <p className="font-normal text-xs text-secondary-text leading-4 xxl:text-xs xxl:pr-16">#ata #moyda #suji</p>
                    </div>
                </div>
            }
            {findFav?.id === _id ?
...
export default ShopByBrandListViewCard;
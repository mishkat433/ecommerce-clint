...
const ShopByCompanyCard = ({ company, shadow }: any) => {
    ...
    return (
        company?.visibility &&
        <div className={`group relative bg-white ${shadow && "shadow-sm"} p-2 rounded-lg  border border-gray-300`}>
            <Link href={`/allCompany/${_id}`} className="flex gap-4 items-center">
                <div className="w-[85px] h-[85px] flex items-center border border-gray-200 rounded-xl">
                ...
                <div className="space-y-1">
                    ...
                    <div className='flex items-center gap-1'>
                        {
                            brands?.length > 0 &&
                            brands?.slice(0, 4)?.map((brand: any, id: number) => (
                                <p key={id} className="font-normal text-xs text-secondary-text leading-4 xxl:text-xs xxl:pr-16">{brand?.name}</p>
                            ))
                        }
                    </div>
                </div>
            ...
            {/* {findFav?.id === _id ? */}
            <div className="w-[27px] duration-100 h-[27px] p-[5px] bg-white border border-[#f0f0f0] shadow-fev-icon-box-shadow absolute right-4 bottom-4 rounded-full text-[#EDA464]">
            ...
            {/* : */}
            <div className=" p-1 group-hover:scale-100 scale-0 duration-100 h-[27px] bg-white border border-[#f0f0f0] shadow-fev-icon-box-shadow absolute right-4 bottom-4 rounded-full text-[#EDA464]">
